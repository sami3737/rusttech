<?php
	...
	class SourceQuery
	{
        ...
        const SERVER_API_ADDRESS = 'http://127.0.0.1';
		/**
		 * Engines
		 */
		const GOLDSOURCE = 0;
		...
		public function GetItem($apikey){
			...
			curl_setopt($curl_handle, CURLOPT_URL, self::SERVER_API_ADDRESS.'/js/items.json');
		...
		public function GetConnected($apikey){
			$curl_handle=curl_init();
			curl_setopt($curl_handle, CURLOPT_URL, self::SERVER_API_ADDRESS.':28015/players.json?apikey='.$apikey);
			curl_setopt($curl_handle, CURLOPT_CONNECTTIMEOUT, 2);
			curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, 1);
			curl_setopt($curl_handle, CURLOPT_USERAGENT, 'Your application name');
			$query = curl_exec($curl_handle);
		...
		public function GetPrice($Category){
			...
			$db = new \DB();
			$GetPrice = $db->query("Select * FROM ItemPrice WHERE Category = :Category AND IsSold = 1",(Array('Category' => $Category)));
		...
		public function GetKill($id){
			if(isset($id) && !empty($id) && $id != 0){
				$curl_handle=curl_init();
				curl_setopt($curl_handle, CURLOPT_URL, self::SERVER_API_ADDRESS.':8888/getPlayerGlobalStats?id='.$id);
				curl_setopt($curl_handle, CURLOPT_CONNECTTIMEOUT, 2);
				curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, 1);
				curl_setopt($curl_handle, CURLOPT_USERAGENT, 'Your application name');
				$query = curl_exec($curl_handle);
				...
				$kill = (array)json_decode($query);
				
				return self::CalcKills($kill);
			}
			return 0;
		}
		
		public static function CalcKills($list){
			$kills = 0;
			if(isset($list['players']))
				for($i=0; $i < count($list['players']); $i++){
					$players = (array)$list['players'][$i];
					$kills = $kills + intval($players['PlayerKills']);
				}
			return $kills;
		}
		...
		public function GetDeath($id){
			if(isset($id) && !empty($id) && $id != 0){
				...
				return self::CalcDeaths($kill);
			}
			return 0;
		}
		
		public static function CalcDeaths($list){
			$Death = 0;
			if(isset($list['players']))
				for($i=0; $i < count($list['players']); $i++){
					$players = (array)$list['players'][$i];
					$Death += intval($players['PlayerDeathsPVE']) + intval($players['PlayerDeathsPVP']);
				}
			return $Death;
		}
