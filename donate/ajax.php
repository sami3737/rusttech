...
if(isset($_POST['checkoutsubmit'])){
		...
		if(isset($_POST['tos-checkbox'])){
			...
			if($sandbox === "0" || ($sandbox === "1" AND $results[0]['usertype'] === "admin") || $paymentMethod === "starpass" || $paymentMethod === "credit"){
				if(empty($results[0]['email'])){
					...
					$sql = $dbcon->prepare("SELECT game, maxpurchases, price, paywhatyouwant, giftable FROM packages WHERE id=:id");
					$values = array(':id' => $packageid);
					$sql->execute($values);
					$results = $sql->fetchAll(PDO::FETCH_ASSOC);
					$gameid = $results[0]['game'];
					$maxpurchases = $results[0]['maxpurchases'];
					$packagePrice = $results[0]['price'];
	...
	if($allowedToPurchase === true){
		...
		if($allowedToPurchase === true){
			...
			$totalPrice = $_SESSION["price"] + $additionalPrice;
			...
			if($paymentMethod === "paypal"){
				...
				if($result === FALSE){
					...
					$sql->execute($values);
					$sql = $dbcon->prepare("INSERT INTO paypalpayments(created, username, price, package, params, paramsdisplay, vars, packagekey, status) VALUES(NOW(), ?, ?, ?, ?, ?, ?, ?, 'waiting')");
					$values = [$_SESSION['username'], $totalPrice, $packageJSON, $paramsJSON, $paramsDisplayJSON, $varsJSON, $_POST['checkoutpackagekey']];
					$sql->execute($values);
					$values = [];
	...
	if(isset($_POST['tos-checkbox'])){
		if(isset($_POST['confirmation-checkbox'])){
			...
			if($credit < $_SESSION['price']){
				...
				$sql = $dbcon->prepare("INSERT INTO transactions(purchaser, usernametype, username, game, expires, expiretime, endcommands, transactionid, package, packageid, paymentmethod, value, status, params) VALUES(:purchaser, :usernametype, :username, :game, :expires, :expiretime, :endcommands, :transactionid, :package, :packageid, :paymentmethod, :value, :status, :params)");
				$values = array(':purchaser' => $_SESSION['username'], ':usernametype' => $usernametype, ':username' => $playerName, ':game' => $game, ':expires' => $package["expires"], ':expiretime' => $expireDate, ':endcommands' => json_encode($endCommands), ':transactionid' => 'Credit Purchase', ':package' => $package['title'], ':packageid' => $package['id'], ':paymentmethod' => "Credits", ':value' => '0.00', ':status' => 'complete', ':params' => json_encode($paramDisplay));
				$sql->execute($values);
