    ...
    if(!isset($_POST['case_type'])){
        if($status === "Completed" OR $status === "Completed-Funds-Held" AND (strtolower($_POST['business']) == $paypalEmail || strtolower($_POST['receiver_email']) == $paypalEmail)){
            if(!empty($_POST["item_number"])){
				...
				$values = array(':id' => $_POST["item_number"]);
				$sql->execute($values);
				$results = $sql->fetchAll(PDO::FETCH_ASSOC);
				...
				if($rowCount > 0){
					if($results[0]['status'] !== 'success'){
						...
						if($_POST['mc_gross'] == $price AND $_POST['mc_currency'] == $currencycode){
							...
							if($sandboxmode == 0){
								...
								$values = array(':purchaser' => $username, ':usernametype' => $usernametype, ':username' => $playerName, ':game' => $game, ':expires' => $package["expires"], ':expiretime' => $expireDate, ':endcommands' => json_encode($endCommands), ':transactionid' => $transactionid, ':package' => $package['title'], ':packageid' => $package['id'], ':paymentmethod' => "PayPal", ':value' => sprintf("%.2f",$price), ':status' => $status, ':params' => json_encode($paramDisplay));
								$sql->execute($values);
							} else {
								$sql = $dbcon->prepare("INSERT INTO transactions(purchaser, usernametype, username, game, expires, expiretime, endcommands, transactionid, package, packageid, paymentmethod, value, status, params) VALUES(:purchaser, :usernametype, :username, :game, :expires, :expiretime, :endcommands, :transactionid, :package, :packageid, :paymentmethod, :value, :status, :params)");
								$values = array(':purchaser' => $username, ':usernametype' => $usernametype, ':username' => $playerName, ':game' => $game, ':expires' => $package["expires"], ':expiretime' => $expireDate, ':endcommands' => json_encode($endCommands), ':transactionid' => $transactionid, ':package' => $package['title'], ':packageid' => $package['id'], ':paymentmethod' => "PayPal", ':value' => sprintf("%.2f",$price), ':status' => $status . ' - Sandbox', ':params' => json_encode($paramDisplay));
								$sql->execute($values);
							}
						}
					}
