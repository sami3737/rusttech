...
$adminsteamid = '76561198127215825';
...
$url = $sdonateapiurl;
$data = array('action' => 'install', 'apikey' => $sdonateapi, 'dir' => $dir);
$options = array(
	'http' => array(
...
if($result === FALSE){
	print("Failed to connect to API server, cancelling installation.");
	exit();
}
switch ($result) {
	case "APIINUSE":
		print("This API key is already in use, deactivate it <a href=\"https://sdonate.com/account.php\">here</a>. Cancelling installation.");
		exit();
	break;

	case "INVALIDAPI":
		print("The API key in your config.php is not valid, cancelling installation.");
		exit();
	break;

	case "APIBANNED":
		print("This API key has been banned, cancelling installation.");
		exit();
	break;
}
...
if(tableExists($dbcon, 'packages') === FALSE){
	$sql = "CREATE table packages(
		...
		img TEXT NOT NULL,
		paywhatyouwant INT(1) NOT NULL,
		price DECIMAL(11,2) NOT NULL,
		maxpurchases INT(11) NOT NULL,
		commands TEXT NOT NULL,
		expires DECIMAL(20,2) NOT NULL,
		enabled INT(1) NOT NULL);";
	$dbcon->exec($sql);
}
...
if(tableExists($dbcon, 'commandstoexecute') === FALSE){
	$sql = "CREATE table commandstoexecute(
		...
		executenow TINYINT NOT NULL DEFAULT 0,
