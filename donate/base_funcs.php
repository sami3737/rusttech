...
function printFooter(){
    print('
        <a href="https://sdonate.com" target="_blank"><p class="footer-text-left">© SDonate Donation System ' . date('Y') . '</p></a>
...
function processCommands($package, $paramValues, $vars, $transactionid, $ready){
        ...
        if($type !== "special"){
            if($game === "Minecraft" OR $game === "Garry's Mod" OR (($game === "Rust" OR $game === "Counter-Strike: Global Offensive" OR $game === "Team Fortress 2" OR $game === "Left 4 Dead 2") AND strpos($value->name, "RCON") === false)){
                ...
                if($value->execute === "onjoin" || $value->execute === "choiceonjoin"){
                    ...
                    if($package["expires"] > 0 && !empty($endCommand)){
...
if(tableExists($dbcon, 'settings') !== FALSE){
    ...
    if(version_compare($version, "1.0.9") === -1){
        $sql = $dbcon->prepare("ALTER TABLE transactions ADD packageid TEXT NOT NULL");
        $sql->execute();
    ...
    if(version_compare($version, "1.1.5") === -1){
        $sql = $dbcon->prepare("ALTER TABLE commandstoexecute ADD ready TINYINT NOT NULL DEFAULT 1");
        $sql->execute();
        $sql = $dbcon->prepare("ALTER TABLE commandstoexecute ADD transactionid TEXT NOT NULL");
        $sql->execute();
    ...
    if(version_compare($version, "1.2") === -1){
        ...
        $sql = $dbcon->prepare("ALTER TABLE users ADD credit DECIMAL(11,2) NOT NULL DEFAULT 0.00");
        $sql->execute();
    ...
    if(version_compare($version, "1.7.0") === -1){
        ...
        $sql->execute($values);
        $sql = $dbcon->prepare("ALTER TABLE packages ADD sortorder INT(11) NOT NULL DEFAULT 1");
        $sql->execute();
    ...
    if(version_compare($version, "1.7.8") === -1){
        ...
        $dbcon->query("ALTER TABLE packages ADD giftable INT(1) NOT NULL DEFAULT 1");
        $sql = $dbcon->prepare("UPDATE settings SET value='1.7.8' WHERE setting='currentversion'");
        $sql->execute();
        $version = "1.7.8";
    }
    if(version_compare($version, "1.7.10") === -1){
        $dbcon->query("ALTER TABLE packages ADD hiddenservers TEXT NOT NULL");
	...
	if(version_compare($version, "1.8.8") === -1){
		$db->query("ALTER TABLE packages ADD cansubscribe INT(1) NOT NULL DEFAULT 0");
