					...
					if(isset($_GET['game']) && !isset($_GET['server'])){
						...
						if($gamecount === 1 && $servercount === 1){
							...
							if($packagecount > 0){
								...
								foreach ($packages as $key => $value) {
									$priceInfo = $currencysymbol . $packages[$key]["price"];
			...
			if (isset($_GET['game']) && isset($_GET['server'])) {
				...
				print('
					...
					function updateTotalPrice(key){
						var price = Number(packages[key].price);
									...
									$.each(packages, function(key, value){
										if(value.id == packageid){
											...
											if(packages[key].paywhatyouwant == 1){
												html += \'<p class="setting-title">' . getLangString("amount-to-pay") . $currencysymbol . '\' + packages[key].price + \')</p>\n\' +
					...
					function showPackage(key){
						...
						var html = \'\' +
							...
							\'<div class="package-price">' . $currencysymbol . '\' + packages[key]["price"] + \' ' . $currencycode . '</div>\n\' +
